...
"""
from models import CostCode, Project, DISCIPLINE_CHOICES
...
class CostCodeService(BaseService):
    ...
    def create_costcode(self, cost_code_id_str, description, discipline, project_id, rule_of_credit_id=None):
        ...
        try:
            ...
            success = self.add(new_costcode)
            if not success:
                print(f"Failed to add cost code to session")
                return None
                
            success = self.commit()
            if not success:
                print(f"Failed to commit cost code to database")
                return None
                
            # Verify the cost code was actually saved by retrieving it
            saved_costcode = self.get_costcode_by_id(new_costcode.id)
            if not saved_costcode:
                print(f"Cost code appears to be created but cannot be retrieved with ID {new_costcode.id}")
