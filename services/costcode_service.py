...
class CostCodeService(BaseService):
    ...
    def create_costcode(self, cost_code_id_str, description, discipline, project_id, rule_of_credit_id=None):
        ...
        try:
            ...
            # Force a refresh of the session to ensure the object is properly persisted
            self.session.refresh(new_costcode)
            return new_costcode
        except Exception as e:
            print(f"Error creating cost code: {str(e)}")
            self.session.rollback()  # Explicitly rollback on error
            return None
