...
import datetime
import io
from reports.pdf_export import generate_quantities_report_pdf, generate_hours_report_pdf
...
@main_bp.route('/export_project_report')
def export_project_report():
    """Export project report"""
    try:
        project_id = request.args.get('project_id', type=int)
        report_type = request.args.get('report_type', 'quantities')
        ...
        filename = f"{project.project_id_str}_{project.name}_{report_type}_{timestamp}"
        ...
        if report_type == 'quantities':
            if export_format == 'pdf':
                ...
                # Excel export will be implemented in the next version
                flash('Excel export is not yet implemented', 'warning')
                return redirect(url_for('main.reports_index'))
        else:  # hours
            if export_format == 'pdf':
                # Generate PDF
...
@main_bp.route('/export_sub_job_report')
def export_sub_job_report():
    """Export sub job report"""
    try:
        sub_job_id = request.args.get('sub_job_id', type=int)
        report_type = request.args.get('report_type', 'quantities')
        export_format = request.args.get('format', 'pdf')
        ...
        # Generate filename
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{project.project_id_str}_{sub_job.name}_{report_type}_{timestamp}"
