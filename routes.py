    ...
    try:
        ...
        if request.method == 'POST':
            ...
            new_work_item = workitem_service.create_workitem(
                work_item_id_str, description, project.id, sub_job_id, cost_code_id,
...
def update_work_item_progress(work_item_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if workitem_service.update_progress_step(work_item_id, updated_progress):
...
@main_bp.route('/api/projects/<int:project_id>/sub_jobs')
...
@main_bp.route('/api/sub_jobs/<int:sub_job_id>/work_items')
def api_sub_job_work_items(sub_job_id):
    ...
    try:
        ...
        return jsonify([{
            ...
            'unit_of_measure': wi.unit_of_measure,
...
@main_bp.route('/api/projects/<int:project_id>/cost_codes')
def api_project_cost_codes(project_id):
    """API endpoint to get all cost codes for a project"""
    try:
        cost_codes = costcode_service.get_costcodes_by_project(project_id)
        return jsonify([{
            'id': cc.id,
            'cost_code_id_str': cc.cost_code_id_str,
...
@main_bp.route('/api/work_items/<int:work_item_id>/progress_history')
def api_work_item_progress_history(work_item_id):
    """API endpoint to get progress history for a work item"""
    try:
        history = workitem_service.get_workitem_progress_history(work_item_id)
