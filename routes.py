...
def list_cost_codes():
    ...
    return render_template('list_cost_codes.html', cost_codes=all_cost_codes, projects=projects, disciplines=disciplines)
...
def add_work_item():
    ...
    if request.method == 'POST':
        try:
            ...
            # Initialize progress for each step in the rule of credit
            ...
            if cost_code and cost_code.rule_of_credit:
                ...
                progress_dict = {}
                for step in steps:
                    progress_dict[step['name']] = 0
                new_work_item.set_steps_progress(progress_dict)
    ...
    # Get pre-selected sub job if coming from sub job view
    ...
    if pre_selected_sub_job_id:
        pre_selected_sub_job = SubJob.query.get(pre_selected_sub_job_id)
        if pre_selected_sub_job:
            pre_selected_project = Project.query.get(pre_selected_sub_job.project_id)
...
def update_work_item_progress(work_item_id):
    ...
    # Get steps and current progress
    steps = rule.get_steps()
    progress = work_item.get_steps_progress()
    ...
    if request.method == 'POST':
        try:
            ...
            for step in steps:
                step_name = step['name']
                step_progress = request.form.get(f'progress_{step_name}')
    ...
    return render_template('update_work_item_progress.html', work_item=work_item, steps=steps, progress=progress)
...
def batch_update_work_items():
    ...
    if request.method == 'POST':
        try:
            # Get work item IDs and progress values
            work_item_ids = request.form.getlist('work_item_id[]')
            progress_values = request.form.getlist('progress[]')
            
            for i in range(len(work_item_ids)):
                work_item_id = int(work_item_ids[i])
                progress = float(progress_values[i])
                ...
                if work_item:
                    ...
                    if cost_code and cost_code.rule_of_credit_id:
                        ...
                        if rule:
                            ...
                            # Update progress for each step
                            for step in steps:
                                work_item.update_progress_step(step['name'], progress)
            
            db.session.commit()
            flash('Work items updated successfully!', 'success')
            return redirect(url_for('main.work_items'))
        except Exception as e:
            db.session.rollback()
            flash(f'Error updating work items: {str(e)}', 'danger')
...
@main_bp.route('/api/project/<int:project_id>/cost_codes')
def api_get_project_cost_codes(project_id):
    ...
    return jsonify([{
        ...
        'has_rule_of_credit': cc.rule_of_credit_id is not None
