...
def sub_jobs():
    ...
    try:
        ...
        if area:
            all_sub_jobs = [sj for sj in all_sub_jobs if hasattr(sj, 'area') and sj.area == area]
        
        # Get unique areas for filtering (safely check if attribute exists)
        areas = sorted(set(sj.area for sj in SubJobService.get_all_sub_jobs() if hasattr(sj, 'area') and sj.area))
...
def add_sub_job():
    ...
    if request.method == 'POST':
        ...
        area = request.form.get('area', '')  # Default to empty string if not provided
    ...
    return render_template('add_sub_job.html', 
                          ...
                          project=selected_project)  # Add project variable for backward compatibility
...
def view_sub_job(sub_job_id):
    ...
    try:
        ...
        # Ensure overall_progress is defined to prevent __round__ error
        overall_progress = metrics.get('percent_complete', 0)
...
def edit_sub_job(sub_job_id):
    ...
    return render_template('edit_sub_job.html', 
                          sub_job=sub_job,
                          project=project)  # Add project variable for consistency
