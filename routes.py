...
def cost_codes():
    try:
        ...
        # Get all cost codes first for debugging
        ...
        if project_id:
            project = ProjectService.get_project_details(project_id)
            # Fixed method name: get_project_cost_codes instead of get_cost_codes_by_project
            ...
            # Always pass all projects for the dropdown
            all_projects = ProjectService.get_all_projects()
            return render_template('cost_codes.html', cost_codes=cost_codes, project=project, disciplines=DEFAULT_DISCIPLINES, projects=all_projects)
        else:
            # If no project_id specified, show all cost codes
            cost_codes = all_cost_codes
            logger.info(f"No project_id specified, showing all {len(cost_codes)} cost codes")
            
            # Always pass all projects for the dropdown
            all_projects = ProjectService.get_all_projects()
            return render_template('cost_codes.html', cost_codes=cost_codes, project=None, disciplines=DEFAULT_DISCIPLINES, projects=all_projects)
    except Exception as e:
...
def add_cost_code():
    try:
        ...
        if request.method == 'POST':
            try:
                project_id = request.form.get('project_id', type=int)
                # Changed parameter name from cost_code_id_str to code to match service method
                ...
                # Create cost code with correct parameter name
                ...
                logger.info(f"Cost code created successfully: ID={cost_code.id}, Code={cost_code.cost_code_id_str}, Project ID={cost_code.project_id}")
                
                flash(f"Cost Code {code} created successfully!", "success")
                # Redirect to cost_codes with the specific project_id to ensure filtering works
