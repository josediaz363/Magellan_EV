...
def add_work_item():
    ...
    if request.method == 'POST':
        description = request.form.get('description')
        project_id = request.form.get('project_id')
        sub_job_id = request.form.get('sub_job_id') or None
        cost_code_id = request.form.get('cost_code_id') or None
        uom = request.form.get('uom')
        budgeted_quantity = request.form.get('budgeted_quantity') or 0
        earned_quantity = request.form.get('earned_quantity') or 0
        budgeted_man_hours = request.form.get('budgeted_man_hours') or 0
        earned_man_hours = request.form.get('earned_man_hours') or 0
        ...
        new_work_item = WorkItem(
            ...
            earned_quantity=float(earned_quantity) if earned_quantity else 0,
            budgeted_man_hours=float(budgeted_man_hours) if budgeted_man_hours else 0,
            earned_man_hours=float(earned_man_hours) if earned_man_hours else 0,
        ...
        # Redirect based on where the work item was added from
        if sub_job_id:
            return redirect(url_for('main.view_sub_job', sub_job_id=sub_job_id))
        else:
            return redirect(url_for('main.view_project', project_id=project_id))
    
    # Get projects, sub jobs, and cost codes for dropdowns
    projects = Project.query.all()
    sub_jobs = []
    cost_codes = CostCode.query.all()
    
    # If project_id is provided in query params, filter sub jobs
...
def edit_work_item(work_item_id):
    ...
    if request.method == 'POST':
        ...
        work_item.sub_job_id = request.form.get('sub_job_id') or None
        work_item.cost_code_id = request.form.get('cost_code_id') or None
        work_item.uom = request.form.get('uom')
        work_item.budgeted_quantity = float(request.form.get('budgeted_quantity') or 0)
        work_item.earned_quantity = float(request.form.get('earned_quantity') or 0)
        work_item.budgeted_man_hours = float(request.form.get('budgeted_man_hours') or 0)
        work_item.earned_man_hours = float(request.form.get('earned_man_hours') or 0)
        
        db.session.commit()
        flash('Work item updated successfully!', 'success')
        
        # Redirect based on where the work item belongs
        if work_item.sub_job_id:
            return redirect(url_for('main.view_sub_job', sub_job_id=work_item.sub_job_id))
        else:
            return redirect(url_for('main.view_project', project_id=work_item.project_id))
...
def delete_work_item(work_item_id):
    ...
    # Redirect based on where the work item belonged
    if sub_job_id:
        return redirect(url_for('main.view_sub_job', sub_job_id=sub_job_id))
    else:
        return redirect(url_for('main.view_project', project_id=project_id))
...
@main_bp.route('/get_sub_jobs/<int:project_id>')
def get_sub_jobs(project_id):
    """Get sub jobs for a project (AJAX)"""
    sub_jobs = SubJob.query.filter_by(project_id=project_id).all()
    return jsonify([{'id': sj.id, 'name': sj.name} for sj in sub_jobs])

@main_bp.route('/get_cost_codes/<int:project_id>')
def get_cost_codes(project_id):
    """Get cost codes for a project (AJAX)"""
    cost_codes = CostCode.query.filter_by(project_id=project_id).all()
    return jsonify([{'id': cc.id, 'name': f"{cc.code} - {cc.description}"} for cc in cost_codes])
...
@main_bp.route('/export_quantities_pdf')
def export_quantities_pdf():
    """Export quantities report as PDF"""
    try:
        from reports import generate_quantities_report_pdf
