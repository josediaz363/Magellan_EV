...
- Fixed projects route to pass projects_with_data structure to template
"""
...
def projects():
    try:
        ...
        for project in projects:
            ...
            sub_jobs = SubJobService.get_project_sub_jobs(project.id) if hasattr(SubJobService, 'get_project_sub_jobs') else []
            ...
            # If there are sub jobs, calculate statistics
            if sub_jobs:
                for sub_job in sub_jobs:
                    # Get work items for this sub job
                    work_items = WorkItemService.get_sub_job_work_items(sub_job.id) if hasattr(WorkItemService, 'get_sub_job_work_items') else []
                    
                    # Calculate hours and progress
                    for work_item in work_items:
                        if hasattr(work_item, 'budgeted_hours'):
                            total_budgeted_hours += work_item.budgeted_hours or 0
                        if hasattr(work_item, 'earned_hours'):
                            total_earned_hours += work_item.earned_hours or 0
