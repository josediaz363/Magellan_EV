...
from models import db, Project, SubJob, RuleOfCredit, CostCode, WorkItem, ProjectCostCode, DISCIPLINE_CHOICES
import json
import uuid
import traceback
import os
    ...
    try:
        if request.method == 'POST':
            try:
                ...
                db.session.flush()  # Flush to get the new cost code ID
        ...
        # Get projects and rules of credit for the form
        projects = Project.query.all()
    ...
    try:
        ...
        if request.method == 'POST':
            try:
                ...
                # Check if code already exists and is not this cost code
                existing_code = CostCode.query.filter_by(cost_code_id_str=code).first()
        ...
        associated_project_ids = [pcc.project_id for pcc in cost_code.projects]
...
def edit_work_item(work_item_id):
    ...
    try:
        ...
        if work_item.cost_code_id not in cost_code_ids:
...
def update_work_item_progress(work_item_id):
    ...
    try:
        ...
        if request.method == 'POST':
            try:
                # Get all step progress values from form
                for key, value in request.form.items():
                    if key.startswith('step_progress_'):
                        step_name = key.replace('step_progress_', '')
                        progress_value = float(value) if value else 0
...
def api_get_cost_codes(project_id):
    ...
    try:
        ...
        return jsonify([{
            ...
            'rule_of_credit_id': cc.rule_of_credit_id
        } for cc in cost_codes])
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@main_bp.route('/api/get_rule_steps/<int:rule_id>')
def api_get_rule_steps(rule_id):
    """API to get steps for a rule of credit"""
    try:
        rule = RuleOfCredit.query.get_or_404(rule_id)
        steps = rule.get_steps()
...
def api_get_project_progress(project_id):
    """API to get overall progress for a project"""
    try:
        # Get all work items for this project
        work_items = WorkItem.query.filter_by(project_id=project_id).all()
...
# Import PDF generation functions from reports module
from reports.pdf_export import QuantitiesPDF, HoursPDF

@main_bp.route('/reports/quantities_pdf/<int:project_id>')
def quantities_pdf(project_id):
    """Generate quantities PDF report for a project"""
    try:
        project = Project.query.get_or_404(project_id)
        
        # Create PDF
        pdf = QuantitiesPDF(project)
        pdf_output = pdf.generate()
        ...
        response = send_file(
            ...
            as_attachment=True,
            download_name=f"{project.project_id_str}_quantities_{datetime.datetime.now().strftime('%Y%m%d')}.pdf"
        )
...
@main_bp.route('/reports/hours_pdf/<int:project_id>')
def hours_pdf(project_id):
    """Generate hours PDF report for a project"""
    try:
        project = Project.query.get_or_404(project_id)
        
        # Create PDF
        pdf = HoursPDF(project)
        pdf_output = pdf.generate()
        ...
        response = send_file(
            ...
            mimetype='application/pdf',
            as_attachment=True,
            download_name=f"{project.project_id_str}_hours_{datetime.datetime.now().strftime('%Y%m%d')}.pdf"
        )
