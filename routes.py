...
# Alias for add_subjob to maintain compatibility with existing templates
@main_bp.route('/project/<int:project_id>/add_sub_job', methods=['GET', 'POST'])
def add_sub_job(project_id):
    """Alias for add_subjob to maintain compatibility"""
    return add_subjob(project_id)
...
# Alias for view_subjob to maintain compatibility
@main_bp.route('/sub_job/<int:subjob_id>')
def view_sub_job(subjob_id):
    """Alias for view_subjob to maintain compatibility"""
    return view_subjob(subjob_id)
...
def edit_subjob(subjob_id):
    ...
    try:
        ...
        return render_template('edit_sub_job.html', subjob=subjob)
    except Exception as e:
        flash(f'Error editing sub job: {str(e)}', 'danger')
        traceback.print_exc()
        return redirect(url_for('main.projects'))

# Alias for edit_subjob to maintain compatibility
@main_bp.route('/edit_sub_job/<int:subjob_id>', methods=['GET', 'POST'])
def edit_sub_job(subjob_id):
    """Alias for edit_subjob to maintain compatibility"""
    return edit_subjob(subjob_id)
...
# Alias for delete_subjob to maintain compatibility
@main_bp.route('/delete_sub_job/<int:subjob_id>', methods=['POST'])
def delete_sub_job(subjob_id):
    """Alias for delete_subjob to maintain compatibility"""
    return delete_subjob(subjob_id)
...
def work_items():
    ...
    try:
        # Get all work items
        all_work_items = workitem_service.get_recent_workitems(100)  # Limit to 100 most recent items
...
@main_bp.route('/api/visualizations/donut/<int:project_id>')
def get_donut_data(project_id):
    ...
    try:
        ...
        percentage = 0
        if total_budgeted_hours > 0:
            percentage = (total_earned_hours / total_budgeted_hours) * 100
        
        # Return data in the format expected by the donut chart
        return jsonify({
            'success': True,
            'data': {
                'percentage': percentage,
                'earned': total_earned_hours,
                'budgeted': total_budgeted_hours
            }
        })
    except Exception as e:
        traceback.print_exc()
