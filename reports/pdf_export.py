...
def generate_quantities_report_pdf(project_id=None, sub_job_id=None):
    ...
    Returns:
        bytes: PDF file as bytes ready for Flask's send_file
    """
    ...
    buffer = io.BytesIO()
    # Use 'F' output mode to ensure proper binary handling
    pdf_data = pdf.output(dest='S')  # Get PDF as string
    
    # Write the PDF data to the buffer
    buffer.write(pdf_data.encode('latin1'))  # Encode as latin1 to preserve binary data
