...
class QuantitiesPDF(FPDF):
    def header(self):
        # Logo
        logo_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'static', 'images', 'magellan_logo_white.png')
        if os.path.exists(logo_path):
            self.image(logo_path, 10, 8, 25)
        
        # Title and subtitle
        self.set_font('Arial', 'B', 16)
        self.cell(0, 10, 'Progress Detail', 0, 0, 'C')
        self.ln(7)
        
        # Get subtitle from the class property
        if hasattr(self, 'report_subtitle'):
            self.set_font('Arial', '', 12)
            self.cell(0, 10, self.report_subtitle, 0, 0, 'C')
    ...
    def table_header(self):
        ...
        # Header
        col_widths = [20, 50, 15, 25, 25, 20, 15, 15, 15, 15, 15, 15, 15]
        ...
        self.cell(col_widths[3], 7, 'Budgeted Quantity', 1, 0, 'C', 1)
        self.cell(col_widths[4], 7, 'Earned Quantity', 1, 0, 'C', 1)
        ...
        # Return to the beginning of the next line and add rule step headers
        self.set_xy(10, self.get_y())
        
        # First row headers
        for i in range(6):
            self.cell(col_widths[i], 7, '', 0, 0)
        
        # We'll add the Rules of Credit step names when we add the cost code rows
        self.ln()
    ...
    def discipline_row(self, discipline):
        ...
        # Discipline row (spans all columns)
        self.cell(0, 6, discipline, 1, 1, 'L', 1)
    ...
    def cost_code_row(self, cost_code, steps):
        ...
        # Cost code info
        col_widths = [20, 50, 15, 25, 25, 20, 15, 15, 15, 15, 15, 15, 15]
        cost_code_text = f"{cost_code.code} - {cost_code.description}"
        ...
        steps_list = cost_code.rule_of_credit.steps if cost_code.rule_of_credit else []
    ...
    def work_item_row(self, item):
        ...
        if len(description) > 40:  # Approximate character limit before wrapping
            # Calculate how many lines we need
            lines = len(description) // 40 + (1 if len(description) % 40 > 0 else 0)
            row_height = 6 * lines  # Adjust row height based on number of lines
        else:
            row_height = 6
        
        # Work item cells
        self.cell(col_widths[0], row_height, str(item.work_item_id), 1, 0, 'L')
        
        # Multi-line cell for description
        current_x = self.get_x()
        current_y = self.get_y()
        self.multi_cell(col_widths[1], row_height / (1 if len(description) <= 40 else lines), description, 1, 'L')
        self.set_xy(current_x + col_widths[1], current_y)
        
        self.cell(col_widths[2], row_height, item.uom or '', 1, 0, 'C')
        ...
        for i in range(7):
            if i < len(progress_steps):
                self.cell(col_widths[i+6], row_height, f"{progress_steps[i].percentage:.0f}%", 1, 0, 'C')
            else:
    ...
    def total_row(self, title, budgeted, earned, is_grand_total=False):
        ...
        # Empty cells for Rules of Credit
        rules_width = sum(col_widths[6:])
...
def generate_quantities_report_pdf(project_id=None, sub_job_id=None):
    ...
    # Create PDF
    pdf = QuantitiesPDF()
    pdf.report_subtitle = report_subtitle
    ...
    for discipline, cost_codes in grouped_items.items():
        ...
        for cost_code_id, data in cost_codes.items():
            ...
            pdf.cost_code_row(data['cost_code'], data['cost_code'].rule_of_credit.steps if data['cost_code'].rule_of_credit else [])
