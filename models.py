...
class Project(db.Model):
    ...
    # Add relationship to ProjectCostCode join table
    project_cost_codes = db.relationship("ProjectCostCode", backref="project", lazy=True, cascade="all, delete-orphan")
    ...
    def get_cost_codes(self):
        """Get all cost codes associated with this project through the join table"""
        return [pcc.cost_code for pcc in self.project_cost_codes]
...
# Join table for many-to-many relationship between Project and CostCode
class ProjectCostCode(db.Model):
    __tablename__ = "project_cost_code"
    id = db.Column(db.Integer, primary_key=True)
    project_id = db.Column(db.Integer, db.ForeignKey("project.id"), nullable=False)
...
class CostCode(db.Model):
    ...
    # Remove direct project_id foreign key to make cost codes independent
    # project_id = db.Column(db.Integer, db.ForeignKey("project.id"), nullable=False)
    ...
    # Add relationship to ProjectCostCode join table
    projects = db.relationship("ProjectCostCode", backref="cost_code", lazy=True, cascade="all, delete-orphan")
    ...
    def get_projects(self):
        """Get all projects associated with this cost code through the join table"""
        return [pcc.project for pcc in self.projects]
